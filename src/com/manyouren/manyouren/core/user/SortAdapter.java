...
public class SortAdapter extends BaseAdapter implements SectionIndexer{
	private List<SortCity> list = null;
	private Context mContext;
	
	public SortAdapter(Context mContext, List<SortCity> list) {
	...
	public View getView(final int position, View view, ViewGroup arg2) {
		...
		final SortCity city = list.get(position);
		if (view == null) {
			viewHolder = new ViewHolder();
			view = LayoutInflater.from(mContext).inflate(R.layout.item_sortlistview, null);
		...
		if(position == getPositionForSection(section)){
			...
			viewHolder.tvLetter.setText(city.getSortLetter());
		}else{
